...
class PasswordHasher:
    ...
    # 1. Recoverable data with Fernet
    print("📁 RECOVERABLE DATA (Fernet)")
    fernet_encryptor = DataEncryptor()
    key = fernet_encryptor.get_key()
    
    data = "Credit card: 1234-5678-9012-3456"
    fernet_encrypted = fernet_encryptor.encrypt(data)
    print(f"Encrypted: {fernet_encrypted[:30]}...")
    
    fernet_decrypted = fernet_encryptor.decrypt(fernet_encrypted)
    print(f"Decrypted: {fernet_decrypted}\n")
    
    # 2. Recoverable data with AES
    print("📁 RECOVERABLE DATA (AES)")
    aes_encryptor = AESDataEncryptor("secret_key_123")
    
    data = "Personal data: Name, email, phone"
    aes_encrypted = aes_encryptor.encrypt_with_salt(data)
    print(f"Encrypted with salt: {aes_encrypted[:30]}...")
    
    aes_decrypted = aes_encryptor.decrypt_with_salt(aes_encrypted)
    print(f"Decrypted: {aes_decrypted}\n")
    
    # 3. Passwords (irreversible) with bcrypt
    print("🔐 PASSWORDS (Irreversible)")
    hasher = PasswordHasher(rounds=12)
    
    password = "MySecurePassword123!"
    
    # Validate strength
    is_strong, message = hasher.is_strong_password(password)
    ...
    # Verify
    is_correct = hasher.verify_password(password, password_hash)
    print(f"Verification: {'✅ Correct' if is_correct else '❌ Incorrect'}")
    
    # Try with incorrect password
    is_correct = hasher.verify_password("wrong_password", password_hash)
    print(f"Verification wrong: {'✅ Correct' if is_correct else '❌ Incorrect'}")
